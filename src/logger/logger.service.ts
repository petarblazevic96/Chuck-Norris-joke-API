...
export class CustomLogger implements LoggerService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const env_configuration =
      this.configService.get<EnvConfiguration>('env_variables');
    ...
    }) as Record<keyof typeof this.nestLogLevels, winston.LeveledLogMethod> &
      winston.Logger;
  }
  ...
  setLogLevels?(levels: LogLevel[]) {
    this.nestLogLevels = levels.reduce(
      (acc: AbstractConfigSetLevels, level: LogLevel) => {
